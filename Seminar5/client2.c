...
void send_message (char* buffer, char* message, int* position)
{
    memcpy (buffer + *position, message, strlen(message));
    memcpy (buffer + *position + strlen(message), "*", 1);
...
int main ()
{
    ...
    recieve_message (buffer1, &ptr_to_read);
    ...
    if (mess_indx != READ)
        send_message (buffer2, message, &ptr_to_write);
